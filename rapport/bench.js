...
for (let a = 0; a < recipes.length; a++) {
	...
	for (let b = 0; b < recipes[a].ingredients.length; b++)
		if (recipes[a].ingredients[b].ingredient.toLowerCase().includes(string)) {
			ingredientFound = true;
		}
	if (
		recipes[a].name.toLowerCase().includes(string) === true ||
		recipes[a].description.toLowerCase().includes(string) === true ||
		ingredientFound === true
	) {
		return true;
	} else {
